...
pub(crate) async fn handle_connection(conn: ConnectionObject, protocol_id: String) -> Result<()> {
...
async fn serve_request(
    ...
    let mut container =
        Container::new(kv, request_headers, request_payload, response_headers, response_payload, Some(res_tx));
                ...
                if let Some(client_upgrade) = container.client_upgrade.take() {
                    let tunnel_future = Box::pin(async move {
                        ...
                        match tokio::try_join!(client_upgrade, upstream_upgrade) {
                            ...
                            Err(e) => {
                                log(LogLevel::Error, &format!("✗ Failed to establish upgrade tunnel: {}", e));
                            },
                        }
                    });
                    payload = PayloadState::Http(VaneBody::UpgradeBridge { tunnel_task: Some(tunnel_future) });
                } else {
                    log(LogLevel::Error, "✗ Response indicates Upgrade, but Client handle is missing!");
